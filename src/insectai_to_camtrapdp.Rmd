...
```{r}
data_raw <- read.csv(here::here("data", "raw", ...))
```
...
```{r}
frictionless::write_package(insectai, here::here("data", "interim", ...))
```
...
```{r}
insectai_dp <- camtrapdp::read_camtrapdp(here::here("data", "interim", ... , "datapackage.json"))
```

# Write Camtrap DP

```{r}
camtrapdp::write_camtrapdp(insectai_dp, here::here("data", "processed", ... ))
```